...
public void Query_SelectAdmin(Database db, DBResultSet results, const char[] error, DataPack pack)
{
    ...
    int iAdminId, iImmunity;
    AdminFlag iFlag;
    AdminId iAdmin;
    char sFlags[33], sIdentity[65], sName[MAX_NAME_LENGTH + 1], sPassword[65], sType[8];
    while (results.FetchRow()) {
        ...
        iImmunity = results.FetchInt(4);
        results.FetchString(5, sFlags,    sizeof(sFlags));
        results.FetchString(6, sPassword, sizeof(sPassword));
        ...
        for (int i = 0, iLen = strlen(sFlags); i < iLen; i++) {
            ...
            SetAdminFlag(iAdmin, iFlag, true);
        }
    }
...
void FetchAdmin(int iClient)
{
    ...
    SB_Escape(sName, sEscapedName, sizeof(sEscapedName));
    Format(sQuery, sizeof(sQuery), "SELECT    ad.aid, ad.user, 'steam', ad.authid, ad.immunity, ad.srv_flags, ad.srv_password, COUNT(ag.group_id) \
                                    FROM      {{admins}}                AS ad \
